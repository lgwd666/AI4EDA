...
// Testcase: testcase_04_client1_write
// Description: Client 1 single write transaction
// Author: lgwd666
// Date: 2026-04-28

`timescale 1ns / 1ps

module testcase_04_client1_write;
    ...
    // ========== Test Sequence ==========
    initial begin
        $display("========== Testcase 04: Client 1 Single Write ==========");
        ...
        #1;
        wait (rst_n);
        $display("[TC04] Starting Client 1 Write transaction");
        $display("[TC04] Address: 0x4000, Data: 0x12345678");
        ...
        @(posedge clk); #1;
        wait (tb_gnt_1);
        $display("[TC04] Grant received from arbiter");

        wait (tb_bvalid_1);
        $display("[TC04] Write completed");
        $display("[TC04] Write response: %b", tb_resp_1);

        // Check result
        if (tb_resp_1 == 2'b00) begin
            $display("[TC04] PASS - Response OKAY");
        end else begin
            $display("[TC04] FAIL - Response: %b", tb_resp_1);
        end

        #50;
        $display("========== Testcase 04 Complete ==========");
        $finish;
    end
    ...
    initial begin
        #5000;
        $display("[TC04] ERROR: Timeout");
        $finish;
    end

    // ========== Waveform Dump ==========
    initial begin
        $dumpfile("testcase_04_client1_write.vcd");
        $dumpvars(0, testcase_04_client1_write);
    end
