...
// Testcase: testcase_01_client0_read
// Description: Client 0 single read transaction
// Author: lgwd666
// Date: 2026-04-28

`timescale 1ns / 1ps

module testcase_01_client0_read;
    ...
    // ========== Test Sequence ==========
    initial begin
        $display("========== Testcase 01: Client 0 Single Read ==========");

        tb_req_0 = 0; tb_cmd_0 = 0; tb_addr_0 = 0; tb_wdata_0 = 0; tb_be_0 = 0;
        tb_req_1 = 0; tb_cmd_1 = 0; tb_addr_1 = 0; tb_wdata_1 = 0; tb_be_1 = 0;

        wait (rst_n);
        $display("[TC01] Starting Client 0 Read transaction");
        $display("[TC01] Address: 0x1000");
        ...
        @(posedge clk); #1;
        wait (tb_gnt_0);
        $display("[TC01] Grant received from arbiter");

        wait (tb_rvalid_0);
        $display("[TC01] Read completed");
        $display("[TC01] Read data: 0x%h, Response: %b", tb_rdata_0, tb_resp_0);

        // Check result
        if (tb_resp_0 == 2'b00) begin
            $display("[TC01] PASS - Response OKAY");
        end else begin
            $display("[TC01] FAIL - Response: %b", tb_resp_0);
        end

        #50;
        $display("========== Testcase 01 Complete ==========");
        $finish;
    end
    ...
    initial begin
        #5000;
        $display("[TC01] ERROR: Timeout");
        $finish;
    end

    // ========== Waveform Dump ==========
    initial begin
        $dumpfile("testcase_01_client0_read.vcd");
        $dumpvars(0, testcase_01_client0_read);
    end
