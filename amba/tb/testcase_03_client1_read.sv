...
// Testcase: testcase_03_client1_read
// Description: Client 1 single read transaction
// Author: lgwd666
// Date: 2026-04-28

`timescale 1ns / 1ps

module testcase_03_client1_read;
    ...
    // ========== Test Sequence ==========
    initial begin
        $display("========== Testcase 03: Client 1 Single Read ==========");
        ...
        #1;
        wait (rst_n);
        $display("[TC03] Starting Client 1 Read transaction");
        $display("[TC03] Address: 0x3000");
        ...
        @(posedge clk); #1;
        wait (tb_gnt_1);
        $display("[TC03] Grant received from arbiter");

        wait (tb_rvalid_1);
        $display("[TC03] Read completed");
        $display("[TC03] Read data: 0x%h, Response: %b", tb_rdata_1, tb_resp_1);

        // Check result
        if (tb_resp_1 == 2'b00) begin
            $display("[TC03] PASS - Response OKAY");
        end else begin
            $display("[TC03] FAIL - Response: %b", tb_resp_1);
        end

        #50;
        $display("========== Testcase 03 Complete ==========");
        $finish;
    end
    ...
    initial begin
        #5000;
        $display("[TC03] ERROR: Timeout");
        $finish;
    end

    // ========== Waveform Dump ==========
    initial begin
        $dumpfile("testcase_03_client1_read.vcd");
        $dumpvars(0, testcase_03_client1_read);
    end
