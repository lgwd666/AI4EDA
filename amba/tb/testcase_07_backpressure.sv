...
// Testcase: testcase_07_backpressure
// Description: AXI backpressure test (awready delayed response)
// Author: lgwd666
// Date: 2026-04-28

`timescale 1ns / 1ps

module testcase_07_backpressure;
    ...
    // ========== AXI BFM with Backpressure ==========
    // awready with configurable delay
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            axi_awready <= 0;
            backpressure_cycles <= 0;
        end else if (axi_awvalid && !axi_awready) begin
            // Wait for backpressure_cycles before asserting ready
            if (backpressure_cycles < 3) begin
    ...
    // ========== Test Sequence ==========
    initial begin
        $display("========== Testcase 07: AXI Backpressure Test ==========");
        ...
        #1;
        wait (rst_n);
        $display("[TC07] Testing backpressure with 3-cycle awready delay");
        ...
        $display("[TC07] Write address channel handshaking with backpressure...");
        wait (tb_bvalid_0);
        $display("[TC07] Write completed despite backpressure");
        $display("[TC07] Write response: %b", tb_resp_0);

        if (tb_resp_0 == 2'b00) begin
            $display("[TC07] PASS - Write completed successfully with backpressure");
        end else begin
            $display("[TC07] FAIL - Response: %b", tb_resp_0);
        end

        #50;
        $display("========== Testcase 07 Complete ==========");
        $finish;
    end
    ...
    initial begin
        #10000;
        $display("[TC07] ERROR: Timeout");
        $finish;
    end

    // ========== Waveform Dump ==========
    initial begin
        $dumpfile("testcase_07_backpressure.vcd");
        $dumpvars(0, testcase_07_backpressure);
    end
